...
{
    if (
        isset($GLOBALS['_OA']['invocationType']) &&
...
function MAX_adRenderImageBeacon($logUrl, $beaconId = 'beacon', $userAgent = null)
{
    ...
    $beaconId .= '_{random}';
    if (
        isset($userAgent) && preg_match("#Mozilla/(1|2|3|4)#", $userAgent)
            ...
            foreach ($aPriorities as $pri) {
                if (!$ad_picked) {
                    $aLinkedAd = OX_Delivery_Common_hook(
                        'adSelect',
                        array(&$aAds, &$context, &$source, &$richMedia, $companion, $type, $pri),
                        $adSelectFunction
                    );
                    if (is_array($aLinkedAd)) {
                        $ad_picked = true;
...
function _adSelect(&$aLinkedAdInfos, $context, $source, $richMedia, $companion, $adArrayVar = 'ads', $cp = null)
{
    ...
    if ($adArrayVar == 'eAds') {
        ...
        if (
            $total_priority_orig > $remaining_priority
            || $companion
        ) {
        ...
        for ($i = 0; $i < $cContext; $i++) {
            ...
            switch ($type) {
                ...
                case 'clientid':
                    switch ($key) {
                        case '!=':
                            $aContext['client']['exclude'][$value] = true;
                            break;
                        case '==':
                            $aContext['client']['include'][$value] = true;
                            break;
                    }
                    break;
                case 'companionid':
