...
function buildBannerForm($type, $aBanner, &$oComponent = null, $formDisabled = false)
{
    ...
    if ($type == 'sql' || $type == 'web') {
        if ($type == 'sql') {
            $header = $form->createElement('header', 'header_sql', $GLOBALS['strMySQLBanner'] . " - " . $GLOBALS['strBannerCreative']);
        } else {
            $header = $form->createElement('header', 'header_sql', $GLOBALS['strWebBanner'] . " - " . $GLOBALS['strBannerCreative']);
        }
        ...
        $form->addElement('header', 'header_b_display', $GLOBALS['strBannerDisplay']);
    ...
    if (OA_Permission::isAccount(OA_ACCOUNT_ADMIN) || OA_Permission::isAccount(OA_ACCOUNT_MANAGER)) {
        $form->addElement('header', 'header_additional',  $GLOBALS['strAdditionalData']);
...
function addUploadGroup($form, $aBanner, $vars)
{
    ...
    if ($update) {
        ...
        if (!empty($vars['decorateId'])) {
            $form->addDecorator($vars['uploadName'] . '_group', 'process', array(
                'tag' => 'tr',
                'addAttributes' => array(
                    'id' => $vars['decorateId'] . '{numCall}',
                    'style' => 'display:none'
                )
            ));
        }
...
function processForm($bannerid, $form, &$oComponent, $formDisabled = false)
{
    ...
    if (
        !empty($_FILES['uploadalt']) && $_FILES['uploadalt']['size'] > 0
